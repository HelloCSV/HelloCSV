...
## indexDBConfig (releasing for `v0.1.2`)
...
- **Different Import Configurations**: If your application has different types of imports (e.g., employee data vs. product data), you can use distinct `customKey` values to manage their states independently.
...
- **Environment Separation**: When running the same application in different environments (development, staging, production), you can use environment-specific keys to prevent state conflicts.
