...
**Type:** `'default' | 'theme-1' | 'theme-2' | 'dark'`

**Required:** No (Default: `default`)

**Description:**
Determines the visual theme of the importer component. Use `dark` for the built-in
dark theme — see [Dark Mode](/v0.5.0/customization/dark-mode). You can further
customize any theme's colors via [Theme Styles](/v0.5.0/customization/theme-styles).
...
**Type:** `'addRows' | 'removeRows' | 'editRows' | 'downloadCsv' | 'search' | 'resetState' | 'backToPreviousStep' | 'undoRedo'[]`

**Required:** No (Default: `['addRows', 'removeRows', 'editRows', 'downloadCsv', 'search', 'resetState', 'backToPreviousStep', 'undoRedo'] - all the actions`)

**Description:**
Determines which actions are available to the user on the preview screen.

The `undoRedo` action enables the Back/Forward toolbar buttons **and** the
undo/redo keyboard shortcuts (see
[Keyboard & Clipboard](/v0.5.0/get-started/keyboard-and-clipboard#undo-redo)).
Omitting it hides the buttons and disables the shortcuts.
